...
from toxapp.fusion_v3 import EvidentialFusionV3, FusionV3Config  # noqa: E402
...
class FeatureDataset(Dataset):
    def __init__(
        ...
        n = len(sequences)
        if plm.shape[0] != n or phys.shape[0] != n or motif.shape[0] != n or msa.shape[0] != n:
...
def collate_features(examples: list[dict]) -> dict:
    ...
    msa = torch.tensor(np.stack([e["msa"] for e in examples], axis=0), dtype=torch.float32)
    return {"sequences": seqs, "labels": y, "plm": plm, "phys": phys, "motif": motif, "msa": msa}


@torch.no_grad()
def predict_loader(model: EvidentialFusionV3, loader: DataLoader, device: torch.device) -> dict[str, np.ndarray]:
    model.eval()
    y_true: List[int] = []
    p_toxic: List[float] = []
    ...
    for batch in loader:
        ...
        out = model(plm, phys, motif, msa, return_att_maps=False)
...
def load_cache(cache_dir: Path, split: str) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    plm = np.load(cache_dir / f"plm_{split}.npy")
    phys = np.load(cache_dir / f"phys_{split}.npy")
    motif = np.load(cache_dir / f"motif_{split}.npy")
    msa = np.load(cache_dir / f"msa_{split}.npy")
    return plm, phys, motif, msa


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--cache-dir", type=str, default="data/feature_cache_v3/protein_toxdl2_v3")
    parser.add_argument("--fusion", type=str, default="trilinear", choices=["tan", "trilinear"])
    ...
    out_dir = Path(args.out_dir) if args.out_dir else Path(f"artifacts/protein_plan_test_v3_toxdl2_{args.fusion}")
    ensure_dir(out_dir)
    ...
    plm_tr, phys_tr, motif_tr, msa_tr = load_cache(cache_dir, "train")
    plm_va, phys_va, motif_va, msa_va = load_cache(cache_dir, "val")
    plm_te, phys_te, motif_te, msa_te = load_cache(cache_dir, "test")
    plm_in, phys_in, motif_in, msa_in = load_cache(cache_dir, "independent")

    train_ds = FeatureDataset(train_seqs, train_y, plm_tr, phys_tr, motif_tr, msa_tr)
    val_ds = FeatureDataset(val_seqs, val_y, plm_va, phys_va, motif_va, msa_va)
    test_ds = FeatureDataset(test_seqs, test_y, plm_te, phys_te, motif_te, msa_te)
    indep_ds = FeatureDataset(indep_seqs, indep_y, plm_in, phys_in, motif_in, msa_in)

    train_loader = DataLoader(train_ds, batch_size=int(args.batch_size), shuffle=True, num_workers=0, collate_fn=collate_features)
    val_loader = DataLoader(val_ds, batch_size=int(args.batch_size), shuffle=False, num_workers=0, collate_fn=collate_features)
    test_loader = DataLoader(test_ds, batch_size=int(args.batch_size), shuffle=False, num_workers=0, collate_fn=collate_features)
    indep_loader = DataLoader(indep_ds, batch_size=int(args.batch_size), shuffle=False, num_workers=0, collate_fn=collate_features)

    cfg = FusionV3Config(
        plm_dim=int(plm_tr.shape[1]),
        phys_dim=int(phys_tr.shape[1]),
        motif_dim=int(motif_tr.shape[1]),
        msa_dim=int(msa_tr.shape[1]),
    ...
    model = EvidentialFusionV3(cfg).to(device)
    ...
    if bool(args.no_class_weights):
        class_weights = None
    else:
        class_weights = torch.tensor([pos / max(1.0, (pos + neg)), neg / max(1.0, (pos + neg))], dtype=torch.float32, device=device)

    best_val = -1e9
    best_epoch = 0
    patience_left = int(args.patience)
    best_path = out_dir / "evi_toxdl2_v3.pt"
    ...
    best_model = EvidentialFusionV3(FusionV3Config(**payload["cfg"])).to(device)
    ...
    # embedding retrieval index (train fused embeddings, cosine)
    train_loader_noshuf = DataLoader(train_ds, batch_size=int(args.batch_size), shuffle=False, num_workers=0, collate_fn=collate_features)
    ...
    # TF-IDF evidence index (train sequences)
    tfidf_index = build_retrieval_index(list(train_out["sequences"].tolist()), list(train_out["y_true"].tolist()))
    save_retrieval_index(tfidf_index, str(out_dir / "retrieval_tfidf.joblib"))
    ...
    summary = {
        ...
        "data": {
            ...
            "train_counts": {"n": int(len(train_y)), "pos": int(sum(int(x) for x in train_y)), "neg": int(len(train_y) - int(sum(int(x) for x in train_y)))},
            "val_counts": {"n": int(len(val_y)), "pos": int(sum(int(x) for x in val_y)), "neg": int(len(val_y) - int(sum(int(x) for x in val_y)))},
            "test_counts": {"n": int(len(test_y)), "pos": int(sum(int(x) for x in test_y)), "neg": int(len(test_y) - int(sum(int(x) for x in test_y)))},
            "independent_counts": {"n": int(len(indep_y)), "pos": int(sum(int(x) for x in indep_y)), "neg": int(len(indep_y) - int(sum(int(x) for x in indep_y)))},
        },
        "model": {"arch": "EvidentialFusionV3(PLM+phys+motif)+MSA-summary+evidential", "cfg": asdict(cfg)},
        "train": {
        ...
        "artifacts": {
            ...
            "independent_predictions": str(out_dir / "independent_predictions.csv"),

...
class PredOutputs:
    ...
    embeddings: np.ndarray  # pooled PLM embeddings [N, D]


@torch.no_grad()
def predict_loader(
    featurizer: EsmFeaturizer, head: EvidentialMLPHead, loader: DataLoader, device: torch.device
) -> PredOutputs:
    head.eval()
    y_true: List[int] = []
    p_toxic: List[float] = []
    ...
    for batch in loader:
        ...
        feats = featurizer.encode(batch.sequences)  # [B, D] float32 on device
        out = head(feats)
...
def main() -> None:
    ...
    parser.add_argument(
        "--plm-path",
        type=str,
        default="/root/group_data/qiuleyu/esm2_t12_35M_UR50D",
    )
    ...
    parser.add_argument("--out-dir", type=str, default="artifacts/protein_plan_test_v1_toxdl2_plm")
    ...
    head = EvidentialMLPHead(
        in_dim=int(featurizer.hidden_size),
        hidden_dim=int(args.adapter_dim),
    ...
    best_path = out_dir / "evi_toxdl2_plm_head.pt"
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in train_loader:
            assert isinstance(batch, RawBatch)
            feats = featurizer.encode(batch.sequences)  # [B, D]
            out = head(feats)
        ...
        val_out = predict_loader(featurizer, head, val_loader, device)
        ...
        if score > best_val:
            ...
            best_epoch = epoch
            patience_left = int(args.patience)
            torch.save({"head_state_dict": head.state_dict(), "head_cfg": {"in_dim": featurizer.hidden_size, "adapter_dim": int(args.adapter_dim), "dropout": float(args.dropout)}}, best_path)
        else:
            patience_left -= 1
            if patience_left <= 0:
    ...
    # Retrieval indices (TF-IDF + PLM embedding cosine)
    ...
    train_out = predict_loader(featurizer, best_head, train_loader_noshuf, device)
    emb_index = build_embedding_index(train_out.embeddings, list(train_out.sequences.tolist()), list(train_out.y_true.tolist()))
    save_embedding_index(emb_index, str(out_dir / "retrieval_plm.joblib"))
    ...
    summary = {
        ...
        "model": {
            "arch": "ESM2-35M(frozen)+MLP-evidential",
        ...
        "artifacts": {
            ...
            "retrieval_plm": str(out_dir / "retrieval_plm.joblib"),
        },
    }

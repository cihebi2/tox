...
class FusionV3Config:
    plm_dim: int = 480
    phys_dim: int = 33
    motif_dim: int = 512
    msa_dim: int = 9
    adapter_dim: int = 32
    fusion: str = "trilinear"  # tan | trilinear
    tan_heads: int = 2
    dropout: float = 0.2
    head_hidden: int = 256
    num_classes: int = 2
...
class EvidentialFusionV3(nn.Module):
    """
    v3 fusion model (sequence-level):
      - PLM pooled embedding (frozen, cached) + adapter
      - physchem global features + adapter
      - motif features + adapter
      - optional MSA summary features + adapter (v2.2)
      - fusion: TAN or trilinear
      - evidential Dirichlet head
    """

    def __init__(self, cfg: FusionV3Config):
        super().__init__()
        self.cfg = cfg
        ...
        self.msa_norm = nn.LayerNorm(int(cfg.msa_dim))
        ...
        self.msa_adapter = nn.Sequential(
            nn.Linear(int(cfg.msa_dim), int(cfg.adapter_dim)),
            nn.ReLU(),
            nn.Dropout(float(cfg.dropout)),
        )
        ...
        if self.fusion == "tan":
            ...
            self.tri_mlp = nn.Sequential(
                nn.Linear(int(cfg.adapter_dim) * 4, int(cfg.adapter_dim)),
                nn.ReLU(),
                nn.Dropout(float(cfg.dropout)),
            )

        self.head = nn.Sequential(
            nn.Linear(int(cfg.adapter_dim), int(cfg.head_hidden)),
            nn.ReLU(),
            nn.Dropout(float(cfg.dropout)),
    ...
    def forward(
        ...
        msa_feat: torch.Tensor,  # [B, msa_dim]
        *,
        ...
        m = self.msa_adapter(self.msa_norm(msa_feat))
        fused = fused + m
